...
- The landing page preview terminal (`docs/index.html` `#demo`) is animated by `docs/js/terminal-demo.js` and styled in `docs/css/main.css` under `.terminal-demo`. It is a multi-scene tour of Ch CLI behavior (direct prompts, pipes, file/URL/web loading, YouTube scraping, codedump, provider/model switching, interactive commands, export, and continuation). Keep the static fallback HTML in sync with the JS scene content for no-JS and reduced-motion users. It auto-follows vertically while animating until the user manually scrolls; if the user scrolls back near the bottom, auto-follow resumes. Hidden-scrollbar vertical scrolling remains available during and after playback, desktop drag-to-scroll works horizontally, and the animation replays on scroll-in only after a 3-minute cooldown. The preview intentionally has no box shadow. Model output streams word-by-word with occasional micro-pauses for a natural token feel while commands and prompts type char-by-char.
