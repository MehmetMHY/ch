...
- The landing page preview terminal (`docs/index.html` `#demo`) is animated by `docs/js/terminal-demo.js` and styled in `docs/css/main.css` under `.terminal-demo`. It is a multi-scene tour of Ch CLI behavior (direct prompts, pipes, file/URL/web loading, YouTube scraping, codedump, provider/model switching, interactive commands, export, and continuation). Keep the static fallback HTML in sync with the JS scene content for no-JS viewers. It auto-follows vertically while animating until the user manually scrolls; if the user scrolls back near the bottom, auto-follow resumes. Hidden-scrollbar vertical scrolling remains available during and after playback, desktop drag-to-scroll works horizontally, and the animation plays once on load/scroll-in, then stops indefinitely until the page is refreshed or the replay button is clicked. The preview intentionally has no box shadow. Model output streams word-by-word with occasional micro-pauses for a natural token feel while commands and prompts type char-by-char.
- Terminal preview controls live in `.terminal-controls` (top-right) and share the `.terminal-btn` class: `#terminal-skip` fast-forwards to the end of the tour and is only present while a run is in flight (`hidden` otherwise), and `#terminal-replay` restarts the tour. Replay is always visible on touch and fades in on hover on desktop; skip stays visible whenever it is relevant because it is time-sensitive. Clicking/tapping a command line copies it to the clipboard (with `execCommand` fallback for insecure contexts) and each command carries a `title="click to copy"` hint. A hint below the preview lists the interactions with separate desktop and touch variants.
- Terminal preview invariants to preserve when editing `terminal-demo.js`:
  ...
  - Skip works by setting a `fastForward` flag that makes `sleep()` resolve immediately and `type`/`typeWords` fill their remaining text in one shot, so the in-flight run drains to exactly the state it would have reached. Do not reimplement skip by rebuilding the DOM.
  - A generation counter (`genId`) cancels in-flight runs on replay so overlapping animations never corrupt the DOM, and only the newest run may clear the `running` flag.
  ...
  - On bfcache restore (back/forward navigation), `pageshow` with `event.persisted === true` calls `start()` to restart the tour cleanly. Without this, stale `fastForward`, burst-firing in-flight timers, and frozen scroll position cause the tour to drain instantly and jump to the bottom.
