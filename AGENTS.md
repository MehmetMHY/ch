...
- `docs/js/terminal-demo.js` - animated multi-scene terminal preview on the landing page.
...
- Every export filename picker (turn, block, manual, and `-e`/`--export` code blocks) supports custom naming via a `>custom` sentinel at the top of the fzf list; selecting it prompts for a name with a short-lived readline prompt. Pressing Enter selects/overwrites the highlighted list item (existing directory files are shown with `[w]` overwrite markers). A query that matches nothing cancels the export, so new filenames are entered only through `>custom` (the old type-to-create path was removed because fzf's fuzzy match almost always matched something, making it impossible to reliably create a new name). Blank filename input, Ctrl+C, and Ctrl+D cancel the export without exiting interactive mode. Custom names are sanitized by stripping `/` only; no extension is auto-appended. `ExportCodeBlocks` also shows existing directory files (with `[w]` overwrite markers) for consistency with the other export modes.
...
- With `save_all_sessions=true`, Ch writes timestamped session files and maintains `~/.ch/tmp/ch_session_latest.ptr`, a small newline-delimited MRU pointer list capped at 10 timestamped session filenames. `-c` tries those entries in order for O(1) recent-session loading, promotes a later valid entry if earlier entries are missing/corrupt, and falls back to scanning `~/.ch/tmp/` only when the pointer is missing or no pointer entry can be loaded. The scan fallback rewrites the pointer for compatibility with older temp dirs.
...
- `internal/chat/chat_test.go` covers the `save_all_sessions=true` latest-session pointer list (`ch_session_latest.ptr`) so `-c` can load a recent timestamped session without scanning the temp directory in normal operation, including missing/corrupt first entries and fallback scan self-healing.
...
- `./.install_test.sh` builds a minimal Ubuntu + Go image (Dockerfile embedded in the script) and runs the real `curl | bash` install from README `main` inside a throwaway container, streaming installer progress while retaining it for the final PASS/FAIL checks. Use it to verify installer changes end-to-end without touching the host. `ensure_govulncheck` is non-fatal on failure since govulncheck is not needed to build and `make security-vuln` falls back to `go run`.
...
- The landing page preview terminal (`docs/index.html` `#demo`) is animated by `docs/js/terminal-demo.js` and styled in `docs/css/main.css` under `.terminal-demo`. It is a multi-scene tour of Ch CLI behavior (direct prompts, pipes, file/URL/web loading, YouTube scraping, codedump, provider/model switching, interactive commands, export, and continuation). Keep the static fallback HTML in sync with the JS scene content for no-JS viewers. It auto-follows vertically while animating until the user manually scrolls; if the user scrolls back near the bottom, auto-follow resumes. Hidden-scrollbar vertical scrolling remains available during and after playback, desktop drag-to-scroll works horizontally, and the animation plays once on load/scroll-in, then stops indefinitely until the page is refreshed. The preview intentionally has no box shadow. Model output streams word-by-word with occasional micro-pauses for a natural token feel while commands and prompts type char-by-char. Clicking/tapping a command line copies it to the clipboard (with `execCommand` fallback for insecure contexts) and each command carries a `title="click to copy"` hint. A hint below the preview lists the interactions with separate desktop and touch variants.
- Terminal preview invariants to preserve when editing `terminal-demo.js`:
  - Hovering must never pause playback. The only pause is a hold while `document.hidden` is true, which exists because browsers throttle background timers to ~1s and would otherwise leave a returning viewer with a crawling, half-finished tour.
  - A generation counter (`genId`) prevents in-flight runs from corrupting the DOM on restart, and only the newest run may clear the `running` flag.
  - `running` guards the scroll-in observer so a tour that is still playing is never restarted underneath itself.
  - `IntersectionObserver` is feature-checked; without it the tour plays once so the preview is never left blank.
  - On bfcache restore (back/forward navigation), `pageshow` with `event.persisted === true` calls `start()` to restart the tour cleanly. Without this, burst-firing in-flight timers and frozen scroll position cause the tour to drain instantly and jump to the bottom.
  - While JS animates, the `<pre>` gets `role="img"` so assistive tech receives the single `aria-label` summary instead of a stream of mutations. No-JS viewers keep the fully readable static text.
