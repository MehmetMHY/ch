...
- Terminal preview controls live in `.terminal-controls` (top-right) and use one `.terminal-btn` button: `#terminal-action` says `skip` while the tour is running and fast-forwards to the end, then says `replay` when the tour is done and restarts it. On desktop/fine pointers, the control fades in on terminal hover or focus; on touch it stays visible. Clicking/tapping a command line copies it to the clipboard (with `execCommand` fallback for insecure contexts) and each command carries a `title="click to copy"` hint. A hint below the preview lists the interactions with separate desktop and touch variants.
